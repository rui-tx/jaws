...
public class BackofficeRepo {
     ...
     */
    public Page<Map<String, String>> getFilteredUsers(PageRequest pageRequest, String status, String role, String search) {
        try {
            ...
            if (status != null && !status.trim().isEmpty()) {
                if ("active".equals(status)) {
                    sqlBuilder.append(" AND u.is_active = 1 AND (u.lockout_until IS NULL OR u.lockout_until < ?)");
            ...
            if (search != null && !search.trim().isEmpty()) {
                sqlBuilder.append(" AND (u.user LIKE ? OR u.email LIKE ? OR u.first_name LIKE ? OR u.last_name LIKE ?)");
    ...
    private Map<String, String> mapUserDetailRow(Row row) {
        ...
        user.put("phone_number", row.get("phone_number") != null ? row.get("phone_number").toString() : "");
        user.put("profile_picture", row.get("profile_picture") != null ? row.get("profile_picture").toString() : "");
        ...
        user.put("last_login", formatTimestamp(row.get("last_login")));
        user.put("failed_login_attempts", row.get("failed_login_attempts") != null ? row.get("failed_login_attempts").toString() : "0");
