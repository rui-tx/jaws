 ...
 * authentication tokens before requests reach controllers.
 */
public class AuthMiddleware implements Middleware {
  ...
  private boolean isAuthenticated(Yggdrasill.RequestContext context) {
    ...
    JawsLogger.debug("AuthMiddleware: Checking authentication for endpoint: {}",
  ...
  /**
   * Check if the request is authorized for the required role(s). Supports comma-separated roles
   * (ANY logic) like "admin,editor"
   */
  ...
  private void sendUnauthorizedResponse(Yggdrasill.RequestContext context, Method routeMethod) {
    try {
      ...
      switch (responseType) {
        ...
        default -> {
          JawsLogger.debug(
              "AuthMiddleware: Sending JSON unauthorized response for unknown response type");
