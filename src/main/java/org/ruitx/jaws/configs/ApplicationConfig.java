 ...
 * application. It loads configuration values from environment variables and a properties file,
 * falling back to default values when necessary.
 */
...
public class ApplicationConfig {
  ...
  public static final List<String> DEFAULT_WWW_BLOCKED_PATTERNS = List.of();
  ...
  public static final List<String> WWW_BLOCKED_PATTERNS;
  ...
  static {
    ...
    WWW_BLOCKED_PATTERNS = getBlockedPatternsValue();
  ...
  private static String getDatabaseSchemaPathValue() {
    return getConfigValue(
        ...
        DEFAULT_DATABASE_SCHEMA_PATH);
  }

  private static String getJWTSecretValue() {
    String jwtSecret = getConfigValue(
        "JWTTOKEN",
        "jwt.secret",
        null);
  ...
  private static boolean getHermodDevelopmentModeValue() {
    String devMode = getConfigValue(
        "HERMOD_DEVELOPMENT_MODE",
        "hermod.development.mode",
        "false");
    return Boolean.parseBoolean(devMode);
  }

  private static long getHermodTemplateCacheTtlValue() {
    String ttl = getConfigValue(
        "HERMOD_TEMPLATE_CACHE_TTL",
        "hermod.template.cache.ttl",
        "3600000");
  ...
  private static List<String> getBlockedPatternsValue() {
    String patterns = getConfigValue(
        "WWW_BLOCKED_PATTERNS",
        "www.blocked.patterns",
        "");
    ...
    List<String> splitPatterns = List.of(patterns.split(","));
    if (!splitPatterns.isEmpty()) {
      return splitPatterns;
    }

    return DEFAULT_WWW_BLOCKED_PATTERNS;
  }
