 ...
 * validates Content-Type headers, parses JSON request bodies, performs deserialization and Jakarta
 * Bean Validation before requests reach controllers.
 */
public class RequestValidationMiddleware implements Middleware {
  ...
  public boolean handle(Yggdrasill.RequestContext context, MiddlewareChain chain) {
    try {
      ...
      // Check if this is an HTMX-only endpoint
      ...
      if (routeMethod != null && routeMethod.isAnnotationPresent(Route.class)) {
        ...
        if (routeAnnotation.htmx()
            && !"true".equalsIgnoreCase(context.getHeader("HX-Request"))) {
      ...
      boolean expectsRequestBody =
          httpMethod.equals("POST") || httpMethod.equals("PUT") || httpMethod.equals("PATCH");

      if (expectsRequestBody && contentType != null && contentType.contains(
      ...
      // Reuse the route method we already found for HTMX check
      if (routeMethod != null) {
        ...
        if (parameterTypes.length > 0) {
          ...
          ValidationResult validationResult = validateAndParseRequestBody(context,
              parameterTypes[0]);
   ...
   */
  private ValidationResult validateAndParseRequestBody(Yggdrasill.RequestContext context,
      Class<?> targetType) {
    String contentType = context.getHeader(CONTENT_TYPE.getHeaderName());
    String requestBodyTrimmed =
        context.getRequestBody() != null ? context.getRequestBody().trim() : "";
    ...
    if (expectsRequestBody) {
      ...
      if (contentType == null) {
        APIResponse<String> response = APIResponse.error(
            ResponseCode.BAD_REQUEST.getCodeAndMessage(),
            "Content-Type header is required for " + httpMethod
                + " requests. Please set Content-Type to 'application/json'"
        );
        sendErrorResponse(context, response);
      ...
      if (requestBodyTrimmed.isEmpty()) {
        APIResponse<String> response = APIResponse.error(
            ResponseCode.BAD_REQUEST.getCodeAndMessage(),
            "Request body is required for " + httpMethod
                + " requests. Please provide a valid JSON body."
        );
        sendErrorResponse(context, response);
        return ValidationResult.failure();
      }
      ...
      try {
        ...
        if (originalMessage != null && originalMessage.contains("Unrecognized field")) {
          ...
          APIResponse<String> response = APIResponse.error(
              ResponseCode.BAD_REQUEST.getCodeAndMessage(),
              String.format(
                  "Unknown field '%s'. Please check the API documentation for accepted fields.",
                  fieldName)
          );
          sendErrorResponse(context, response);
          return ValidationResult.failure();
  ...
  /**
   * Find the route method for the given endpoint and request type. This uses the same logic as
   * AuthMiddleware for consistency.
   */
  private Method findRouteMethod(String endPoint, RequestType requestType) {
    JawsLogger.debug(
        "RequestValidationMiddleware: Finding route method for endpoint: {} and request type: {}",
  ...
  /**
   * Extracts the field name from Jackson's error message for unknown fields. Example: "Unrecognized
   * field \"content\" (class ...)" -> "content"
   */
   ...
   */
  private boolean sendErrorResponse(Yggdrasill.RequestContext context,
    ...
    try {
      ...
      JawsLogger.error("RequestValidationMiddleware: Error sending validation error response: {}",
          e.getMessage());
      ...
      try {
        ...
        JawsLogger.error("RequestValidationMiddleware: Failed to send fallback error response: {}",
