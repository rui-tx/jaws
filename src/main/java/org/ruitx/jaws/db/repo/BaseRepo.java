package org.ruitx.jaws.db.repo;

import java.util.ArrayList;
import java.util.List;
import java.util.Optional;
import org.ruitx.jaws.components.Mimir;
import org.ruitx.jaws.db.mapping.RowMapper;
 ...
 * Keep it simple and SQLite-first for now.
 */
public abstract class BaseRepo {
  protected final Mimir db;

  protected BaseRepo(Mimir db) {
    this.db = db;
  }
  ...
  // Transactions
  @FunctionalInterface
  public interface SqlSupplier<R> { R get() throws Exception; }

  @FunctionalInterface
  public interface SqlRunnable { void run() throws Exception; }
  ...
  protected void inTransaction(SqlRunnable body) throws Exception {
    inTransaction(() -> { body.run(); return null; });
  }
