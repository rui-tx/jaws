...
public class JobRetryManager {
   ...
   */
  public RetryDecision shouldRetry(String jobId, String jobType, Throwable exception,
  ...
  public RetryDecision shouldRetry(String jobId, Throwable exception, int currentRetries,
      int maxRetries) {
   ...
   */
  private long calculateAdvancedRetryDelay(int retryCount,
      JobErrorClassifier.ClassificationResult classification) {
    ...
    Logger.debug(
        "Error classification retry delay calculation: base={}ms, multiplier={}, jitter={}ms, final={}ms (attempt {})",
  ...
  /**
   * Calculate exponential backoff delay with jitter (fallback method) Formula: baseDelay *
   * (exponentialBase ^ retryCount) + jitter Jitter prevents thundering herd when many jobs fail
   * simultaneously
   */
  ...
  /**
   * Schedule a job for retry by updating database fields This method prepares the job for retry
   * without immediately re-queuing it
   */
  ...
  /**
   * Mark a job as permanently failed This is called when retry limits are exceeded or permanent
   * errors occur
   */
  ...
  /**
   * Get the current retry count for a job from the database This is needed because jobs are
   * reconstructed from the database during retry
   */
  ...
  public static class RetryDecision {
    ...
    public JobErrorClassifier.ErrorType getErrorType() {
      return errorType;
    }
    ...
    public int getSuggestedMaxRetries() {
      return suggestedMaxRetries;
    }
  }
  ...
  public static class RetryStatistics {
    ...
    public RetryStatistics(int retryScheduledJobs, int permanentlyFailedJobs,
        int totalRetryAttempts) {
    ...
    public int getRetryScheduledJobs() {
      return retryScheduledJobs;
    }

    public int getPermanentlyFailedJobs() {
      return permanentlyFailedJobs;
    }
