...
/**
 * JobErrorClassifier - Classifies job errors and provides retry strategies
 */
public class JobErrorClassifier {
   ...
   */
  private ErrorType classifyByExceptionType(Throwable exception, String jobType,
   ...
   * Determine retry strategy based on error type and pattern
   */
  private RetryStrategy determineRetryStrategy(ErrorType errorType, ErrorPattern pattern,
      String jobType) {
  ...
  private RetryStrategy getBaseRetryStrategy(ErrorType errorType) {
    return switch (errorType) {
      ...
      case TRANSIENT_RATE_LIMIT ->
          new RetryStrategy("rate_limit", 2, 30000L, 4.0); // Long delays for rate limits
      case TRANSIENT_SERVICE_UNAVAILABLE ->
          new RetryStrategy("service_unavailable", 4, 10000L, 3.0);
      case TRANSIENT_DATABASE -> new RetryStrategy("database", 3, 1000L, 2.0);
      case TRANSIENT_RESOURCE_EXHAUSTION ->
          new RetryStrategy("resource_exhaustion", 2, 15000L, 3.0);
      case UNKNOWN_INVESTIGATE ->
          new RetryStrategy("unknown", 1, 5000L, 2.0); // Conservative for unknown
   ...
   */
  private String generateClassificationReason(ErrorType errorType, ErrorPattern pattern,
  ...
  public static class ClassificationResult {
    ...
    public long getSuggestedBaseDelayMs() {
      return suggestedBaseDelayMs;
    }

    public double getSuggestedBackoffMultiplier() {
      return suggestedBackoffMultiplier;
    }
