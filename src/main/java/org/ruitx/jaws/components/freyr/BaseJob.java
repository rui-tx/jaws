 ...
 * properties such as ID, type, priority, max retries, timeout, execution mode, and payload.
 * <p>
...
public abstract class BaseJob implements Job {
   ...
   * @param payload    the job payload containing additional data
   */
  protected BaseJob(String type, int priority, int maxRetries, long timeoutMs,
      Map<String, Object> payload) {
    this(type, ExecutionMode.DEFAULT, priority, maxRetries, timeoutMs, payload);
  }

  /**
   * Constructor for jobs with execution mode and default settings, defaults to PARALLEL It defaults
   * to priority 5, 3 retries, and 30 seconds timeout.
   *
   * @param type          the job type
   * @param executionMode the execution mode (e.g., PARALLEL, SEQUENTIAL)
   * @param payload       the job payload containing additional data
  ...
  /**
   * Constructor for jobs with default settings, defaults to PARALLEL It defaults to DEFAULT
   * execution mode, priority 5, 3 retries, and 30 seconds timeout.
   *
   * @param type    the job type
   * @param payload the job payload containing additional data
   */
  protected BaseJob(String type, Map<String, Object> payload) {
    this(type, ExecutionMode.DEFAULT, 5, 3, 30000L,
        payload); // Default: priority 5, 3 retries, 30s timeout
  }
  ...
  /**
   * Helper method to get typed data from payload This method checks if the value exists and is of
   * the expected type.
   *
  ...
  /**
   * Helper method to get string data from payload This method retrieves the value associated with
   * the key and casts it to String.
   *
   * @param key the key to look up in the payload
  ...
  /**
   * Helper method to get integer data from payload This method retrieves the value associated with
   * the key and converts it to Integer.
   *
   * @param key the key to look up in the payload
  ...
  /**
   * Helper method to get long data from payload This method retrieves the value associated with the
   * key and converts it to Long.
   *
   * @param key the key to look up in the payload
  ...
  /**
   * Helper method to get boolean data from payload This method retrieves the value associated with
   * the key and converts it to Boolean.
   *
   * @param key the key to look up in the payload
  ...
  public String toString() {
    return String.format("Job{id='%s', type='%s', priority=%d, mode=%s}", id, type, priority,
        executionMode);
  }
} 