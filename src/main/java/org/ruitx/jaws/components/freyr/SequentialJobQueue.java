...
public class SequentialJobQueue {
  ...
  public SequentialJobQueue(DeadLetterQueue sharedDeadLetterQueue) {
    ...
    Logger.info("SequentialJobQueue initialized with queue capacity of {} and shared DLQ",
  ...
  /**
   * Shutdown the sequential processing system Waits for current job to complete before shutting
   * down
   */
  ...
  private class SequentialJobWorker implements Runnable {
    ...
    /**
     * Run the sequential job worker.
     *
     * <p>This method continuously polls the sequential job queue for new jobs to process. If a job
     * is available, it invokes the processJob method to execute it. The thread will run as long as
     ...
     *
     * <p>This method handles the execution of a job in a sequential processing context. It updates
     * the job status in the database and handles exceptions by applying retry logic. If a job fails
     * permanently or exceeds retries, it is moved to the dead letter queue.
     *
    ...
    private void processJob(Job job) {
      ...
      try {
        ...
        updateJobStatus(job.getId(), Freyr.JobStatus.PROCESSING, null, Instant.now().toEpochMilli(),
            null);
        job.execute();
        updateJobStatus(job.getId(), Freyr.JobStatus.COMPLETED, null, null,
        ...
        if (decision.shouldRetry()) {
          ...
          Logger.info("Sequential job {} scheduled for retry: {}", job.getId(),
              decision.getReason());
        } else {
          ...
          Logger.warn("Sequential job {} permanently failed and moved to DLQ: {}", job.getId(),
              decision.getReason());
        }
    ...
    /**
     * Update the status of a job in the database, along with additional information: error message,
     ...
     */
    private void updateJobStatus(String jobId, Freyr.JobStatus status, String errorMessage,
