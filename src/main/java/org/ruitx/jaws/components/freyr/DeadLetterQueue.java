...
public class DeadLetterQueue {
  ...
  /**
   * Move a permanently failed job to the Dead Letter Queue This is called when a job exhausts all
   * retry attempts or fails with permanent errors
   *
  ...
  /**
   * Manually retry a job from the Dead Letter Queue This allows administrators to retry jobs after
  ...
  public DLQStatistics getStatistics() {
    try {
      ...
      List<Row> typeRows = mimir.getRows(
          "SELECT job_type, COUNT(*) as count FROM DEAD_LETTER_QUEUE GROUP BY job_type");
  ...
  public Map<String, String> batchRetry(List<String> dlqEntryIds, boolean resetRetryCount) {
    ...
    Logger.info("Batch retry completed: {} successful out of {} total", successful,
        dlqEntryIds.size());
  ...
  public static class DLQEntry {
    ...
    public List<String> getRetryHistory() {
      return retryHistory;
    }
  ...
  public static class DLQStatistics {
    ...
    public int getRetryableEntries() {
      return retryableEntries;
    }

    public Map<String, Integer> getEntriesByType() {
