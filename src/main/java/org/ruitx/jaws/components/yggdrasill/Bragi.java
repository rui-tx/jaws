...
import org.ruitx.jaws.enums.ParamType;
...
public abstract class Bragi {
  ...
  /**
   * Updates the template variables using the provided context map. Each entry in the context map is
   * set as a template variable.
   *
   * @param context a map containing key-value pairs where the key represents the template variable
   *                name and the value represents the associated variable value to be set.
   */
   ...
   * @param data the response data
   */
  protected void sendSuccess(APIResponse<Object> data) {
    ResponseCode responseCode = ResponseCode.fromCodeAndMessage(data.code());
    sendJSONResponse(data.success(), responseCode, data.info(), data.data());
  }
  ...
  protected Part file(String name) {
    Yggdrasill.RequestContext context = requestContext.get();
    return context != null && context.getMultipartFiles() != null ? context.getMultipartFiles()
        .get(name) : null;
  }
   ...
   */
  protected String processTemplate(String templatePath, Context templateContext)
      throws IOException {
    Yggdrasill.RequestContext context = requestContext.get();
    if (context != null) {
   ...
   */
  public <T> APIResponse<T> call(String endpoint, RequestType method, Object body,
   ...
   * @return the parsed API response
   */
  public <T> APIResponse<T> call(String endpoint, RequestType method, Map<String, String> headers,
   ...
   */
  public <T> APIResponse<T> call(String endpoint, RequestType method, Map<String, String> headers,
      Object body, JavaType responseType) {
    String jsonBody = body != null ? encode(body) : null;
  ...
  private <T> APIResponse<T> parseResponse(String response, Class<T> responseClass) {
    try {
      ...
      return APIResponse.error(ResponseCode.BAD_REQUEST.getCodeAndMessage(),
          "Invalid JSON response");
    } catch (Exception e) {
      JawsLogger.error("Failed to parse API response: {}", e.getMessage());
      return APIResponse.error(ResponseCode.INTERNAL_SERVER_ERROR.getCodeAndMessage(),
