 ...
 * <p>
 * Scenario: 1. Create a fresh in-memory database with one table. 2. Enable cache → first SELECT
 * should populate cache (MISS → PUT). 3. Same SELECT again should be served from cache (HIT) – row
 * count unchanged. 4. Perform an INSERT (write) which targets the same table → cache entry must be
 * invalidated automatically. 5. Third SELECT must reflect the new row (cache repopulated)
 */
@TestInstance(TestInstance.Lifecycle.PER_CLASS)
public class MimirCacheTest {

  private static final String DB_PATH = "cachetest.mimir";
  private Mimir db;
