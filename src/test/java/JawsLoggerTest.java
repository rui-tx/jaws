...
public class JawsLoggerTest {
  ...
  public void testJawsLoggerWithArguments() {
    ...
    JawsLogger.debug("Processing request for user: {} at timestamp: {}", username,
  ...
  public void testDatabasePersistence() {
    ...
    try {
      ...
      List<Row> logEntries = logsDb.getRows(
          "SELECT COUNT(*) as count FROM LOG_ENTRIES WHERE message LIKE '%persistence%'");
      ...
      if (!logEntries.isEmpty()) {
        ...
        List<Row> recentLogs = logsDb.getRows(
            "SELECT level, message, logger, method FROM LOG_ENTRIES WHERE message LIKE '%persistence%' ORDER BY timestamp DESC LIMIT 5");
