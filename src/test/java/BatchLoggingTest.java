...
public class BatchLoggingTest {
  ...
  public static void cleanup() {
    try {
      ...
      new File("src/main/resources/logs.mimir").delete();
  ...
  private void verifyDatabaseEntries(int expectedMinimum) {
    try {
      ...
      List<Row> logEntries = logsDb.getRows(
          "SELECT COUNT(*) as count FROM LOG_ENTRIES WHERE message LIKE '%Performance test%' OR message LIKE '%Debug message%'");
      ...
      if (!logEntries.isEmpty()) {
        ...
        List<Row> sampleEntries = logsDb.getRows(
            "SELECT level, message, logger, method FROM LOG_ENTRIES WHERE message LIKE '%test%' ORDER BY timestamp DESC LIMIT 5");
